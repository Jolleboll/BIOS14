...
plantsmat=as.matrix(plants)[,-c(1,2)]


# plantsmat
# var(plantsmat)
correlations=cor(plantsmat)
ztransformed=scale(plantsmat)
# quit()
...
pca1=princomp(plantsmat)
summary(pca1)
pca1$loadings

pca2=princomp(correlations)
summary(pca2)
pca2$loadings

pca3=princomp(ztransformed)
summary(pca3)
pca3$loadings

svg('arrrrplot.svg',width=18,height=8,pointsize=16)
par(mfrow=c(1,3))
biplot(pca1, col=c("grey", "black"), cex=c(.5, 1))
biplot(pca2, col=c("grey", "black"), cex=c(.5, 1))
biplot(pca3, col=c("grey", "black"), cex=c(.5, 1))
dev.off()


temps=plantsmat[,1:6]
ztemps=scale(temps)
pcaT=princomp(ztemps)
summary(pcaT)
loadings(pcaT)
...
others=plantsmat[,-c(1:6)]
zothers=scale(others)
pcaO=princomp(zothers)
summary(pcaO)
loadings(pcaO)
...
# reduced_fc=Carex.bigelowii~pcaT1+pcaT2+light+snow+soil_moist+altitude
m_fc=lm(plants$Carex.bigelowii ~ pcaT1+pcaT2+plants$light+plants$snow+plants$soil_moist+plants$altitude)
m_fc_full=lm(full_fc,data=plants)

summary(m_fc)
summary(m_fc_full)

# So basically, we do PCA to reduce the dimensionality of the predictors, which is useful if we have
# too many to meaningfully interpret, or we risk overfitting. Using PCs instead of the raw predictors
# will always reduce the model fit scores, like R^2, but the predictive power of the model can be much
# better.
